    ...
    for msg_count in [10, 100, 1000, 10000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, &msg_count| {
                ...
                b.iter(|| {
                    proxy.start_recording_internal("ws-messages".to_string()).expect("Failed to start");
...
fn bench_websocket_replay(c: &mut Criterion) {
    ...
    group.bench_function("replay_websocket_cassette", |b| {
        ...
        proxy.start_recording_internal("ws-replay-test".to_string()).expect("Failed to start");
        proxy.stop_recording_internal().expect("Failed to stop");
        ...
        b.iter(|| {
            black_box(proxy.replay_internal("ws-replay-test".to_string()).expect("Failed to replay"));
        });
    });
